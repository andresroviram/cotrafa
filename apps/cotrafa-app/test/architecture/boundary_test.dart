  ...
  group('architecture boundary policy', () {
      ...
      final BoundaryScanReport report =
          scanArchitectureSources(<ArchitectureSource>[
            const ArchitectureSource(
              ...
              content: "import 'package:cotrafa_app/app.dart';",
            ),
            const ArchitectureSource(
            ...
            const ArchitectureSource(
              ...
              content:
                  "import '../../../../apps/cotrafa-app/lib/app.dart';\n"
                  "import 'package:cotrafa_database/"
                  "cotrafa_database.dart';",
            ),
          ]);
    ...
    test('canonical transition and final architectures are accepted', () {
      final BoundaryScanReport p2 = scanArchitectureSources(
        _sources(
          "database/lib/cotrafa_database.dart|import 'package:drift/drift.dart'; class CotrafaDatabase {}|database/lib/tables/users.dart|part of '../cotrafa_database.dart'; class Users {}|database/lib/injectable.dart|import 'package:injectable/injectable.dart';|app/lib/config/injectable/injectable_dependency.dart|import 'package:cotrafa_database/injectable.module.dart';",
        ),
      );
      ...
      final violations = <BoundaryViolation>[
        ...scanArchitectureSources(
          _sources(
            "database/lib/cotrafa_database.dart|import 'package:drift/drift.dart';",
          ),
        ).violations,
        ...p2.violations,
    ...
    final List<String> rejectedFixtures = <String>[
      "feature app import|featureDoesNotImportApp|src/auth/domain/usecases/login.dart|import 'package:cotrafa_app/config/database/database_module.dart';",
      ...
      "public barrel concrete leak|publicBarrelLeaksPersistence|auth.dart|export 'src/database/cotrafa_database.dart';",
      'final feature database|finalFeatureHasInfrastructure|src/database/cotrafa_database.dart|',
      ...
      'final feature Drift type|featureDoesNotImportDrift|src/auth/domain/entities/bad_identity.dart|final CotrafaDatabase database;',
      ...
      "P7 obsolete persistence|finalFeatureHasInfrastructure|injectable.dart|void configureDependencies() {}|app/lib/config/injectable/injectable_dependency.dart|import 'package:feature_auth/injectable.module.dart';|src/database/cotrafa_database.g.dart|class _GeneratedDatabase {}",
    ];
...
const String _requiredAppPersistence =
    'database/lib/cotrafa_database.dart,database/lib/injectable.dart,database/lib/tables/users.dart,database/lib/tables/login_identifiers.dart,database/lib/tables/addresses.dart,database/lib/tables/transfers.dart,database/lib/tables/local_session.dart,app/lib/config/injectable/injectable_dependency.dart';
...
BoundaryScanReport scanArchitectureWorkspace(Directory workspace) {
  ...
  final List<ArchitectureSource> sources = <ArchitectureSource>[
    ...
    _readSource(workspace, 'apps/cotrafa-app/pubspec.yaml', 'app/pubspec.yaml'),
    _readSource(
      workspace,
...
BoundaryScanReport scanArchitectureSources(
  ...
  for (final ArchitectureSource source in sources) {
    ...
    if (_isFeatureManifest(path) &&
        (source.content.contains('cotrafa_app:') ||
            source.content.contains('apps/cotrafa-app'))) {
      violations.add(
        _sourceViolation(BoundaryRule.featureManifestDependsOnApp, path),
      );
    }
    if (source.content.contains('ICotrafaDatabase') ||
    ...
    if (enforceFinal &&
        ...
        (source.content.contains('CotrafaDatabase') ||
    ...
    if (appSource &&
        source.content.contains('cotrafa_database.dart') &&
        RegExp(
...
bool _importsApplication(String importUri) =>
    importUri.startsWith('package:cotrafa_app/') ||
    importUri.contains('apps/cotrafa-app/') ||
    importUri.contains('apps/cotrafa_app/');
...
bool _importsPersistence(String importUri) =>
    importUri.startsWith('package:drift/') ||
    importUri.endsWith('/cotrafa_database.dart') ||
    importUri == 'cotrafa_database.dart';
...
bool _leaksPersistence(String importUri) =>
    importUri.contains('cotrafa_database') ||
...
bool _leaksPersistenceContent(String content) => RegExp(
  r'''\b(?:QueryRow|GeneratedDatabase|CotrafaDatabase|DataClass|AddressesData|LocalSessionData)\b|package:drift/|(?:export|import)\s+['"][^'"]*(?:\.g\.dart|cotrafa_database)[^'"]*['"]''',
).hasMatch(content);
...
Directory findWorkspaceRoot() =>
    File('apps/cotrafa-app/pubspec.yaml').existsSync()
    ? Directory.current
  ...
  for (final String path in _requiredAppPersistence.split(','))
    ArchitectureSource(
      ...
      content: path.endsWith('injectable_dependency.dart')
          ? "import 'package:cotrafa_database/injectable.module.dart';"
